...
@RestController
@Controller
@RequestMapping("/api/stockAudit")
public class ManagerStockAuditAction {

    private static Logger logger = LoggerFactory.getLogger(ManagerStockAuditAction.class);
    ...
    public WebPageResponse queryAuditList(@RequestParam(defaultValue = "0") Integer offset, @RequestParam(defaultValue = "10") Integer limit,@RequestParam(defaultValue = "") String name) {
        WebPageResponse response =new WebPageResponse();
        StockAuditParam stockAuditParam =new StockAuditParam();
        if(StringUtils.isNotEmpty(name)){
            stockAuditParam.setName(name);
        }
        try {
