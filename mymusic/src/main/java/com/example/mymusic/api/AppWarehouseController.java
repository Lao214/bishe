...
public class AppWarehouseController {
    ...
    private static Logger logger = LoggerFactory.getLogger(AppWarehouseController.class);
    ...
    public WebPageResponse queryWarehouseList(@RequestParam(defaultValue = "0") Integer start, @RequestParam(defaultValue = "4") Integer limit, @RequestParam(defaultValue = "0") Integer sort) {
        ...
        warehouseParam.setSort(sort);
        List<Warehouse> pages=  warehouseService.queryWarehouseByConditions(warehouseParam);
     ...
     * @Author 劳威锟
     */
    @GetMapping(value = "/queryWarehouseByAddress")
    public WebPageResponse queryWarehouseByAddress(@RequestParam(defaultValue = "")String province,@RequestParam(defaultValue = "")String city,@RequestParam(defaultValue = "0")String area) {
        WebPageResponse response =new WebPageResponse();
        WarehouseParam warehouseParam =new WarehouseParam();
        warehouseParam.setLimit(1000);
        try {
            warehouseParam.setProvince(province);
            warehouseParam.setCity(city);
            warehouseParam.setArea(area);
            List<Warehouse> pages=  warehouseService.queryWarehouseByConditionApp(warehouseParam);
            response.setData(pages);
            response.setStatus(WebResponseCode.SUCCESS);
            response.setMsg("查询成功");
        } catch (FdServiceException e) {
