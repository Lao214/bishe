  ...
  <div style="margin:10px 0">
     ...
     >
      <template slot-scope="scope">
        <el-popover trigger="hover" placement="top">
          <p>地区: {{ scope.row.stockAuditName }}</p>
    ...
    <el-table-column label="操作" width="200px"  align="center">
      <template slot-scope="scope">
        ...
        <!--  <el-popconfirm title="这是一段内容确定删除吗？"  @confirm="handleDelete(scope.row.stockAuditNo)">
        <el-button slot="reference" size="mini"  type="danger"  v-if="admin.adminLevel===2" >删除</el-button>
          </el-popconfirm>-->
        <el-popconfirm title="确定通过此申请吗？"  @confirm="handlePass(scope.row.stockAuditNo)">
        <el-button slot="reference" size="mini"  type="success"  v-if="admin.adminLevel===2&&scope.row.status==='待审核'" >通过</el-button>
          </el-popconfirm>
       <el-popconfirm title="确定驳回此申请吗？"  @confirm="handleReject(scope.row.stockAuditNo)">
        <el-button slot="reference" size="mini"  type="danger"  v-if="admin.adminLevel===2&&scope.row.status==='待审核'" >驳回</el-button>
          </el-popconfirm>
      </template>
    </el-table-column>
  </el-table>
...
<!--添加表单-->
  ...
  <el-form :model="form">
    <el-form-item label="物品名" :label-width="formLabelWidth">
      ...
      <el-input v-model="form.stockAuditName" autocomplete="off"></el-input>
    </el-form-item>
     ...
     <el-form-item label="数量" :label-width="formLabelWidth">
      <el-input v-model="form.count" autocomplete="off"></el-input>
       ...
       <el-form-item label="操作类型（出/入）" :label-width="formLabelWidth">
  ...
  <el-form :model="forms">
    <el-form-item label="物品名" :label-width="formLabelWidth">
      ...
      <el-input v-model="forms.secondType"  autocomplete="off"></el-input>
       ...
       <el-form-item label="地区" :label-width="formLabelWidth">
      <el-input v-model="forms.stockAuditName"  autocomplete="off"></el-input>
    </el-form-item>
      <el-form-item label="品牌" :label-width="formLabelWidth">
...
export default {
    ...
    created()
    {
      ...
      {
          loadTable(){
              request.get("/stockAudit/queryAuditList",{
                params:{
        ...
        save(){       
         ...
         request.post("/stockAudit/addStockAudit",this.form).then(res =>{
             if(res.status===1){
             this.$message({
             message: '添加成功',
               type: 'success'
      ...
      edit(){
          request.post("/stockAudit/updateStockAudit",this.forms).then(res =>{
           console.log(res)
           if(res.status===1){
             this.$message({
             message: '更新成功',
      ...
      handleDelete(stockAuditNo) {
           this.stockAuditNo=stockAuditNo
           request.post("/stockAudit/deleteStockAudit/"+this.stockAuditNo).then(res =>{
           console.log(res)
           console.log(this.stockAuditNo)
           if(res.status===1){
             this.$message({
             message: '删除成功',
               type: 'success'
      ...
      handlePass(stockAuditNo) {
           this.stockAuditNo=stockAuditNo
           request.post("/stockAudit/stockAuditPass/"+this.stockAuditNo).then(res =>{
           console.log(res)
           console.log(this.stockAuditNo)
           if(res.status===1){
             this.$message({
           ...
           this.loadTable();
            ...
            handleReject(stockAuditNo) {
           this.stockAuditNo=stockAuditNo
           request.post("/stockAudit/stockAuditReject/"+this.stockAuditNo).then(res =>{
           console.log(res)
           console.log(this.stockAuditNo)
           if(res.status===1){
             this.$message({
